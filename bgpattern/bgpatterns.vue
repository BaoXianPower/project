...
<script type="text/ecmascript-6">
    ...
    let canvasPos = {left: 100, top: 100};
    ...
    export default {
        ...
        methods: {
            ...
            colorize(canvas, img, color) {
                ...
                ctx.fillStyle = this.rgbaFilter([255, 255, 255, 0]); // 设置或返回用于填充绘画的颜色、渐变或模式
                ctx.fillRect(0, 0, img.width, img.height); // 绘制“已填色”的矩形
                ...
                ctx.fillStyle = this.rgbaFilter(color);
            ...
            imageLoad (image, e) {
                ...
                if (img.complete) {
                    image._clrUrl = colorize(image.color); // base64位
                }
            },
            ...
            postLink () {
                ...
                let x, y, i, params, img, steps, destWidth, destHeight, gridStep, snapshots, order = [-1, 0, 1];
            ...
            bgpTransformBox () {
                ...
                if (target) {
                    ...
                    this.elCss = {
                        left: canvasPos.left + target.offsetX * this.pattern.width * this.pattern.scale + bounds.cx + 'px',
                        top: canvasPos.top + target.offsetY * this.pattern.height * this.pattern.scale + bounds.cy + 'px'
                    }
